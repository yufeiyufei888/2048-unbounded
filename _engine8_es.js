...
// ---------- 评估（进化权重表模式：R0-R3 行表 + T 列表 + dEmpty 固定修正）----------
const fs8 = require(String.fromCharCode(102)+String.fromCharCode(115));
let WT_R0 = null, WT_R1 = null, WT_R2 = null, WT_R3 = null, WT_T = null, WT_DE1 = 400000, WT_DE2 = 60000;
let WT_DR0 = null, WT_DR1 = null, WT_DR2 = null, WT_DR3 = null, WT_DT = null;
let WT_MONO = 3.0, WT_SMOOTH = 6.0, WT_EMPTY = 80000;
function loadWeightTable(p) {
  const buf = fs8.readFileSync(p);
  const F = new Float64Array(buf.buffer, buf.byteOffset, 5 * 65536);
  WT_R0 = F.slice(0, 65536); WT_R1 = F.slice(65536, 131072); WT_R2 = F.slice(131072, 196608); WT_R3 = F.slice(196608, 262144); WT_T = F.slice(262144, 327680);
  const meta = JSON.parse(fs8.readFileSync(p + ".meta.json", "utf8"));
  WT_DE1 = meta.dEmpty1; WT_DE2 = meta.dEmpty2;
  WT_MONO = meta.wMono || 3.0; WT_SMOOTH = meta.wSmooth || 6.0; WT_EMPTY = meta.wEmpty || 80000;
  if (meta.hasDelta) {
    const D = new Float64Array(buf.buffer, buf.byteOffset + 5 * 65536 * 8, 5 * 65536);
    WT_DR0 = D.slice(0, 65536); WT_DR1 = D.slice(65536, 131072); WT_DR2 = D.slice(131072, 196608); WT_DR3 = D.slice(196608, 262144); WT_DT = D.slice(262144, 327680);
  }
}
try { loadWeightTable(require("path").join(__dirname, "evolve_tables", "current.bin")); } catch (e) { console.error("[engine8_es] 未找到 current.bin，用零表"); }

function evaluate(lo, hi) {
  if (!WT_R0) return 0;
  const r0 = lo & 0xFFFF, r1 = (lo >>> 16) & 0xFFFF, r2 = hi & 0xFFFF, r3 = (hi >>> 16) & 0xFFFF;
  // 参数化蛇形主项 + 特征（与 v7.4 语义一致，表值来自增量引擎的原生表）
  ...
  const smooth = ROW_SMOOTH[r0] + ROW_SMOOTH[r1] + ROW_SMOOTH[r2] + ROW_SMOOTH[r3]
  ...
  s += mono * WT_MONO - smooth * WT_SMOOTH + empty * WT_EMPTY;
  if (empty <= 1) s -= WT_DE1;
  else if (empty === 2) s -= WT_DE2;
  // Δ 加性修正表（CMA-ES 进化对象；初始 0 = v7.4）
  if (WT_DR0) s += WT_DR0[r0] + WT_DR1[r1] + WT_DR2[r2] + WT_DR3[r3] + WT_DT[c0] + WT_DT[c1] + WT_DT[c2] + WT_DT[c3];
  return s;
}
