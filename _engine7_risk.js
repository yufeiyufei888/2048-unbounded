/*
 * 2048 引擎 v7.2risk —— ttfix + 风险敏感 Expectimax + 采样模式（任务4/任务5）
 * ==========================================================================
 * 【风险敏感随机节点】
 *   基线在随机节点只取期望值 mean = (1/n)Σ v_i，对"坏运气分支"与"好运气分支"
 *   一视同仁 → 高阶阶段容易过度押注单一大方块路线，一次坏运气就崩盘。
 *   本版本在随机节点计算：
 *       result = mean - riskLambda * risk
 *   risk 的两种度量（riskMode 按位组合）：
 *     mode 1（半偏差 / downside semideviation，主方案）：
 *       risk = (1/n) Σ max(0, mean - v_i)
 *       —— 下行偏差，天然涵盖：空位骤减、可行动方向减少、合并机会丢失、
 *          大方块被坏运气顶离蛇形前端等一切"值变差"的情形。
 *     mode 2（特征风险，探索项，默认权重 0）：
 *       对每个 90% 分支的子局面直接计特征罚（value 单位，按
 *       scale = 2^maxExp * base^15 缩放）：
 *         - 子局面空位 ≤ 1 → riskWDead * scale
 *         - 子局面可行动方向 ≤ 1 → riskWMob * scale（=2 时半罚）
 *         - 最大方块脱离蛇形前端(SNAKE_POS≠0) → riskWHead * scale
 *   【早期保护】riskPhase（默认 12）：仅当局面最高方块 ≥ 2^riskPhase(=4096)
 *   才启用风险项，避免破坏前期合并效率；设 0 表示全程启用。
 *
 * 【随机节点采样模式 sampleMode】（继承自 ttfix，任务5）
 *     0 = 无重复分层抽样（v7.1 基线）
 *     1 = 有放回均匀抽样
 *     2 = 按蛇形位置重要性加权抽样
 *
 * 搜索/评估/移动逻辑与 _engine7_ttfix.js 完全一致（含平坦精确键 TT、种子机制）。
 * riskLambda=0 且各 riskW*=0 时，行为与 ttfix 完全等价（仅多一次 VALS 写入）。
 */
'use strict';

const ENGINE_TAG = 'v7.2-risk';
...
let P = {
  ...
  sampleMode: 0,
  // 风险敏感参数（任务4）
  riskLambda: 0,    // λ：风险厌恶系数，0 = 关闭（与 ttfix 等价）
  riskMode: 1,      // 1=半偏差 2=特征风险 3=两者
  riskPhase: 12,    // maxExp ≥ riskPhase 才启用；0 = 恒启用
  riskWDead: 0,     // 特征：子局面空位≤1（scale = 2^maxExp·base^15）
  riskWMob: 0,      // 特征：子局面可行动方向≤1
  riskWHead: 0      // 特征：最大方块脱离蛇形前端
};
...
buildCellW();
// 风险计算缓冲
const RISK_VALS = new Float64Array(32);
const RISK_BL = new Uint32Array(32);   // 90% 分支棋盘（用于特征风险）
const RISK_BH = new Uint32Array(32);

function maxExpOf(lo, hi) {
  let m = 0, r;
  r = lo & 0xFFFF;          { const e=(r>>>12)&0xF; if(e>m)m=e; const f=(r>>>8)&0xF; if(f>m)m=f; const g=(r>>>4)&0xF; if(g>m)m=g; const h=r&0xF; if(h>m)m=h; }
  r = (lo>>>16)&0xFFFF;     { const e=(r>>>12)&0xF; if(e>m)m=e; const f=(r>>>8)&0xF; if(f>m)m=f; const g=(r>>>4)&0xF; if(g>m)m=g; const h=r&0xF; if(h>m)m=h; }
  r = hi & 0xFFFF;          { const e=(r>>>12)&0xF; if(e>m)m=e; const f=(r>>>8)&0xF; if(f>m)m=f; const g=(r>>>4)&0xF; if(g>m)m=g; const h=r&0xF; if(h>m)m=h; }
  r = (hi>>>16)&0xFFFF;     { const e=(r>>>12)&0xF; if(e>m)m=e; const f=(r>>>8)&0xF; if(f>m)m=f; const g=(r>>>4)&0xF; if(g>m)m=g; const h=r&0xF; if(h>m)m=h; }
  return m;
}
function childFeatureRisk(l2, h2) {
  // 90% 分支子局面的特征风险（value 单位）
  const emp = countEmpty(l2, h2);
  let fr = 0;
  if (emp <= 1) fr += P.riskWDead;
  // 可行动方向数（行+列 LUT）
  const r0 = l2 & 0xFFFF, r1 = (l2>>>16)&0xFFFF, r2 = h2 & 0xFFFF, r3 = (h2>>>16)&0xFFFF;
  const c0 = ((r0 & 0xF000)) | ((r1 & 0xF000) >>> 4) | ((r2 & 0xF000) >>> 8) | ((r3 & 0xF000) >>> 12);
  const c1 = ((r0 & 0x0F00) << 4) | (r1 & 0x0F00) | ((r2 & 0x0F00) >>> 4) | ((r3 & 0x0F00) >>> 8);
  const c2 = ((r0 & 0x00F0) << 8) | ((r1 & 0x00F0) << 4) | (r2 & 0x00F0) | ((r3 & 0x00F0) >>> 4);
  const c3 = ((r0 & 0x000F) << 12) | ((r1 & 0x000F) << 8) | ((r2 & 0x000F) << 4) | (r3 & 0x000F);
  const mob =
    ...
    (MOVE_LUT[c2] !== c2 || RIGHT_LUT[c2] !== c2 ? 1 : 0) +
    (MOVE_LUT[c3] !== c3 || RIGHT_LUT[c3] !== c3 ? 1 : 0);
  if (mob <= 1) fr += P.riskWMob; else if (mob === 2) fr += P.riskWMob * 0.5;
  // 最大方块是否在蛇形前端
  let maxE = 0, bestRank = 15;
  for (let rr = 0; rr < 4; rr++) {
    const row = rr === 0 ? r0 : rr === 1 ? r1 : rr === 2 ? r2 : r3;
    for (let cc = 0; cc < 4; cc++) {
      const e = (row >>> (12 - 4 * cc)) & 0xF;
      if (e > maxE) { maxE = e; bestRank = SNAKE_POS[rr * 4 + cc]; }
      else if (e === maxE && e > 0 && SNAKE_POS[rr * 4 + cc] < bestRank) bestRank = SNAKE_POS[rr * 4 + cc];
    }
  }
  if (maxE > 0 && bestRank !== 0) fr += P.riskWHead;
  return fr;
}
    ...
    if (!nEmp) result = exitimax(lo, hi, depth - 1, true, limit);
      ...
      if (P.sampleMode === 1) {
        for (let i = 0; i < n; i++) {
          ...
          const v = 0.9 * exitimax(l2, h2, depth - 1, true, limit) + 0.1 * exitimax(l4, h4, depth - 1, true, limit);
          RISK_VALS[i] = v; RISK_BL[i] = l2; RISK_BH[i] = h2;
          sum += v;
        }
      ...
      const mean = sum / n;
      // 风险项：riskLambda>0 且（riskPhase=0 或 局面 maxExp 达到门槛）才计算
      if (P.riskLambda > 0 && (P.riskPhase === 0 || maxExpOf(lo, hi) >= P.riskPhase)) {
        let risk = 0;
        if (P.riskMode & 1) {
          for (let i = 0; i < n; i++) { const d = mean - RISK_VALS[i]; if (d > 0) risk += d; }
          risk /= n;
        }
        if (P.riskMode & 2) {
          const scale = POW2[maxExpOf(lo, hi)] * Math.pow(P.base, 15);
          for (let i = 0; i < n; i++) risk += childFeatureRisk(RISK_BL[i], RISK_BH[i]) * scale;
          risk /= n;
        }
        result = mean - P.riskLambda * risk;
      } else {
        result = mean;
      }
    }
...
module.exports = { playGame, setParams, getParams: () => P, evaluate, readCells, CELLS, bestMove, moveDir, setBoard: (l,h)=>{B_LO=l>>>0;B_HI=h>>>0;}, getBoard: ()=>({lo:B_LO,hi:B_HI}), countEmpty, getGain: () => G, setSeed, clearSeed, maxExpOf, ENGINE_TAG };
