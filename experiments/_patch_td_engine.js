// _patch_td_engine.js —— 对生成的 _engine7_td.js 打增强补丁（每次 _mk_td.js 重新生成后需重跑）
// 内容：① evaluateFused 加 tdBlendUp 上限守卫（高位防外推垃圾）
//       ② setTdNet 改用兄弟校准（决策粒度 K；随机盘校准曾致搜索崩溃 tdscout03）
//       ③ 内置轻量 moveBoard LUT 供兄弟生成
//       ④ 参数 tdRelWeight 默认 0.01、tdBlendUp 默认 12
const fs = require('fs');
const F = '_engine7_td.js';
let s = fs.readFileSync(F, 'utf8');
let n = 0;
function rep(from, to) {
  if (!s.includes(from)) { console.error('未找到:', JSON.stringify(from.slice(0, 80))); process.exit(1); }
...
// 4) 参数
rep("  tdNetPath: ''      // 权重文件路径（eval2 的 params JSON 注入用）\n};",
...
console.log('已打补丁 _engine7_td.js，替换数', n);
