...
let P = {
  ...
  after13nn: 0,
  after13nnExp: 13,
  // C1 快速实验：双链潜力项（maxExp≥wChain2From 时对次链就绪度加分）
  // g_align = 与主块相邻且指数=主块-1 的方块数（随时可合成次大块）
...
// ---------- 评估（折叠查表 + 可选双尺度）----------
const CHAIN_E = new Int8Array(16);    // wChain2 扫描缓冲
const MAIN_MASK = new Int8Array(16);  // 主块位置掩码
function evaluate(lo, hi) {
  ...
  const chainOn = P.wChain2 > 0;
  const needMax = dualOn || nnOn || chainOn || P.wMobility || P.wMerge || P.wCorner || P.wDeadEnd;
  const SCAN_E = chainOn ? CHAIN_E : null;
  const mainMask = chainOn ? MAIN_MASK : null;
  let maxExp = 0, bestRank = 15, comp = 0;
  if (needMax) {
    if (mainMask) mainMask.fill(0);
    for (let r = 0; r < 4; r++) {
      const row = r === 0 ? r0 : r === 1 ? r1 : r === 2 ? r2 : r3;
      for (let c = 0; c < 4; c++) {
        const e = (row >>> (12 - 4 * c)) & 0xF;
        if (SCAN_E) SCAN_E[r * 4 + c] = e;
        if (e === 0) continue;
        const rank = SNAKE_POS[r * 4 + c];
        if (e > maxExp) {
          maxExp = e; bestRank = rank;
          comp = (dualOn && e >= P.dualStartExp) ? POW2[e] * (BR_W[rank] - LR_W[rank]) : 0;
          if (mainMask) { mainMask.fill(0); mainMask[r * 4 + c] = 1; }
        } else if (e === maxExp) {
          if (rank < bestRank) bestRank = rank;
          if (dualOn && e >= P.dualStartExp) comp += POW2[e] * (BR_W[rank] - LR_W[rank]);
          if (mainMask) mainMask[r * 4 + c] = 1;
        }
      }
    }
  }
  ...
  // C1：双链潜力项（次链就绪度，仅默认评价路径；与 dual/nn 实验分支互斥）
  if (chainOn && maxExp >= P.wChain2From && !(dualOn && maxExp >= P.dualStartExp) && !(nnOn && maxExp >= P.after13nnExp)) {
    const m1 = maxExp - 1, m2 = maxExp - 2;
    let gAlign = 0, gLane = 0;
    for (let i = 0; i < 16; i++) {
      const e = SCAN_E[i];
      if (e !== m1 && e !== m2) continue;
      const r = (i / 4) | 0, c = i % 4;
      if (e === m1) {
        const adj = (c > 0 && mainMask[i - 1]) || (c < 3 && mainMask[i + 1])
                 || (r > 0 && mainMask[i - 4]) || (r < 3 && mainMask[i + 4]);
        if (adj) gAlign++;
        gLane += (16 - SNAKE_POS[i]) / 16;
      } else {
        gLane += (16 - SNAKE_POS[i]) / 32;
      }
    }
    s += P.wChain2 * (gAlign + 0.3 * gLane);
  }
  return s;
}
